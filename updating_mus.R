...
x <- rnormmix(300, lambda = c(0.45, 0.25, .3), mu = c(-4, 1, 5), sigma = c(1, 1, 1))
...
#---PROPOSAL---#
muProposal <- function(xt, s = 1) {
  # random walk (symmetric) proposal
  r.cand = rnorm(1, mean = xt, sd = s)
  # they will be the same, proposal is symmetrical
  d.cand = dnorm(r.cand, mean = xt, sd = s, log = T)
  d.curr = dnorm(xt, mean = xt, sd = s, log = T)
...
#---SAMPLER---#
###############
metropolisHastings <- function(loglikelihood, prior, proposal, data, z.start, mu.start, P, mu0, P0, alpha, Nsim) {
  
  N <- length(z.start)
  K <- length(mu.start)
  
  chain = array(dim = c(Nsim, N + K))
  chain[1, 1:N] = z.start
  chain[1, (N+1):(N+K) ] = mu.start
  
  for (i in 1 : (Nsim - 1)) {
    ### Proposal for the vector of means
    mucandidate = unlist(sapply(chain[i, (N+1):(N+K)], muProposal, s = P)[1,])        
        
    ### The class indicators
    Zcandidate = zProposal(z = chain[i, 1:N], K, N, mu = mucandidate , P,  mu0, P0, alpha)
    
    r.candidate = Zcandidate$r.cand
    d.candidate = Zcandidate$d.cand
    d.curr = Zcandidate$d.curr
    
    probab = exp(
      (loglikelihood(mu = mucandidate, z = r.candidate, P = P, data) + zPrior(r.candidate, K, N, mu = mucandidate, mu0, P0, alpha) + d.candidate) -
        (loglikelihood(mu = chain[i, (N+1):(N+K)], z = chain[i, ], P = P, data) + zPrior(chain[i, ], K, N, mu = chain[i, (N+1):(N+K)], mu0, P0, alpha) + d.curr)
    )
    
    if (runif(1) < probab) {
      chain[i + 1, 1:N ] = r.candidate
      chain[i + 1, (N+1):(N+K)] = mucandidate
    } else {
      chain[i + 1, 1:N] = chain[i, 1:N ]
      chain[i + 1, (N+1):(N+K)] = chain[i, (N+1):(N+K)]
    }#END: accept check
...
run <- function() {
  ...
  alpha <- 0.01
  z     <- rep(1, N)
  mu    <- rep(0, 3)  # c(2, -4) # messing around with the starting values, just for fun :-)
  K     <- length(mu)
  mu0   <- mean(x) 
  P0    <- sd(x)
  burnin <- 200
  
  chain = metropolisHastings(loglikelihood, prior, proposal, data = x, z.start = z, mu.start = mu, P, mu0, P0, alpha, Nsim)
