...
partialLoglike <- function(x, mu, P) {
  return(dnorm( x, mean = mu, sd = P, log = TRUE))
}
...
loglikelihood <- function(mu, z, P, data) {
  ...
  for(i in 1 : length(data)) {
    xi = data[i]
    mui = mu[z[i]]
    logL = logL + partialLoglike(xi, mui, P) #dnorm(x = xi, mean = mui, sd = P, log = TRUERUE )
  }
...
#---PROPOSAL---#
muProposal <- function(xt, operate = TRUE, gaussian = FALSE, window = 0.1, s = NULL) {
  # random walk (symmetric) proposals 
  K = length(xt)
  r.cand = rep(NA, K)
  d.cand = rep(NA, K)
  d.curr = rep(NA, K)
  ...
  for(i in 1 : K) {
    ...
    if(operate) {
      if (gaussian){
        ...
        r.cand[i] = rnorm(1, mean = xt[i], sd = s)
        d.cand[i] = dnorm(r.cand[i], mean = xt[i], sd = s, log = TRUE)
        d.curr[i] = dnorm(xt[i], mean = xt[i], sd = s, log = TRUE)
        
      }else{
        
        r.cand[i] = runif(1, min = xt[i] - window, max = xt[i] + window)
        # they will be the same, proposal is symmetrical
        d.cand[i] = dunif(r.cand[i], min = xt[i] - window, max = xt[i] + window, log = TRUE)
        d.curr[i] = dunif(xt[i], min = xt[i] - window, max = xt[i] + window, log = TRUE)
  ...
  for(index in 1 : N) {
    ...
    for(i in 1 : K) {
      ...
      if(occupancy[i] == 0) {# draw new
        ...
        like = dnorm( x[index], mu[i], P, log = TRUE) 
...
#---SAMPLER---#
###############
metropolisHastings <- function(loglikelihood, prior, proposal, data, zstartvalue, mustartvalue, P, mu0, P0, alpha, Nsim, GP = FALSE) {
  ...
  for (i in 1 : (Nsim - 1)) {
    ...
    mucandidate = muProposal(muchain[i, ], operate = TRUE, gaussian = GP, s = P) 
  ...
  return(list(zchain = zchain, muchain = muchain, acceptance.ratio = mean(acc)))
...
run <- function() {
  ...
  chain = metropolisHastings(loglikelihood, prior, proposal, data = x, zstartvalue = z, mustartvalue = mu, P, mu0, P0, alpha, Nsim, GP = FALSE)
  
  muchain = chain$muchain
  zchain = chain$zchain
  
  cat("Acceptance ratio:", chain$acceptance.ratio, "\n\n")
